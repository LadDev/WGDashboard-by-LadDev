...
const Navdata = () => {
    ...
    const [isWireGuard, setIsWireGuard] = useState(false);
    ...
    const [wgItems, setWgItems] = useState([])
    ...
    useEffect(() => {
        if (configsRes !== configs) {
            ...
            for (const config of configsRes) {
                items.push({
                    id: `configuration-${config.conf}`,
                    label: `${config.conf}`,
                    link: `/wireguard/${config.conf}`,
                    parentId: "wireguard",
                })
            }
    ...
    const menuItems = [
        ...
        },
        {
            ...
            icon: "bx bx-world",
            link: "/#",
            stateVariables: isWireGuard,
            click: function (e) {
                e.preventDefault();
                setIsWireGuard(!isWireGuard);
                setIscurrentState("WireGuard");
                updateIconSidebar(e);
            },
            subItems: wgItems,
        },
            ...
            subItems: [
                {
                    ...
                    click: function (e) {
                        e.preventDefault();
                        const button = document.getElementById('show_ping_modal');
                        button.click()
                    },
                },
                {
                    ...
                    click: function (e) {
                        e.preventDefault();
                        const button = document.getElementById('show_traceroute_modal');
                        button.click()
                    },
                }
            ]
