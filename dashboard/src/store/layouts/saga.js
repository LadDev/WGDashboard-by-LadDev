...
import {
    ...
    CHANGE_PRELOADER,
    CHANGE_SIDEBAR_VISIBILITY, CHANGE_LANG
} from './actionType';
import {postChangeLang, postChangeTheme, postLogin} from "../../helpers/backend_helper";
...
function* changeLayoutMode({ payload: mode }) {
    try {
        ...
        if(authUser && authUser.theme !== mode){
            authUser.theme = mode
            localStorage.setItem("authUser", JSON.stringify(authUser))
            yield call(postChangeTheme, {theme: mode});
        }
    } catch (error) {
        // console.log(error);
    }
...
function* changeLanguage({ payload: language }) {
    try {
        yield call(postChangeLang, {language: language});
    } catch (error) {
        console.log(error);
    }
}
...
export function* watchchangeChangeLanguage() {
    yield takeEvery(CHANGE_LANG, changeLanguage);
}
...
function* LayoutSaga() {
    yield all([
        ...
        fork(watchchangeChangeLanguage)
    ]);
}
